...
function getHost(): string {
    const h = U.GetConfig('title_hostname');
...
export const BeginMatching = async (data: any) => {
    ...
    if (room === null) {
        ...
        room = { roomId, version, user: parseInt(newMember.userId), restMSec: 60, isFull: false, matchingMemberInfoList: [newMember] };
        putRoom(room);
    } else {
...
export const EndMatching = async (data: any) => {
    ...
    if (!room) return { matchingResult: 1, matchingMemberInfoList: [], matchingMemberRoleList: [], reflectorUri: '' };

    const members = room.matchingMemberInfoList;
    const roleList = members.map(m => ({ role: parseInt(m.userId) === room.user ? 1 : 0 }));
...
export const RemoveMatchingMember = async (data: any) => {
    ...
    for (const room of getAllRooms(version)) {
        ...
        room.matchingMemberInfoList = room.matchingMemberInfoList.filter(m => m.userId !== data.userId);
        if (room.matchingMemberInfoList.length === before) continue;
        if (room.matchingMemberInfoList.length <= 0) deleteRoom(version, room.roomId);
        else putRoom(room);
        break;
    }
    return { returnCode: '1' };
...
export const GetMatchingState = async (data: any) => {
    ...
    if (!room) {
        return {
            roomId: data.roomId,
            matchingWaitState: { isFinish: true, restMSec: 0, pollingInterval, matchingMemberInfoList: [] },
        };
    }
    ...
    const idx = room.matchingMemberInfoList.findIndex(m => m.userId === currentMember.userId);
    if (idx >= 0) room.matchingMemberInfoList[idx] = currentMember;

    putRoom(room);

    const diffMembers = room.matchingMemberInfoList.filter(m => m.userId !== currentMember.userId);
