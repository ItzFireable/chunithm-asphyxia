...
function roomKey(version: number, roomId: number): string {
  return `${version}:${roomId}`;
}
...
export const EndMatching = async (data: any) => {
  ...
  const roleList = members.map(m => ({
                                 role: parseInt(m.userId) === room.user ? 1 : 0,
                               }));
  ...
  return {
    ...
    reflectorUri: 'naominet.jp',
  };
};

export const RemoveMatchingMember = async (data: any) => {
  const version = 18;
  const allRooms = getAllRooms(version);
...
export const GetMatchingState = async (data: any) => {
  ...
  if (idx >= 0) {
    room.matchingMemberInfoList[idx] = currentMember;
  }
