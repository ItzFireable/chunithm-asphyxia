...
function rebootTimes() {
    const fmt = (d: Date) => d.toISOString().replace('T', ' ').replace(/\.\d+Z$/, '.0');
    const now = Date.now();
    return { start: fmt(new Date(now + 6 * 3600 * 1000)), end: fmt(new Date(now + 7 * 3600 * 1000)) };
}

function matchTimes() {
    const fmt = (d: Date) => d.toISOString().replace('T', ' ').replace(/\.\d+Z$/, '.0');
    const now = Date.now();
    return { start: fmt(new Date(now + 8 * 3600 * 1000)), end: fmt(new Date(now + 16 * 3600 * 1000)) };
}
...
export const GetGameUCCondition       = async (data: any) => ({ length: 0, gameUnlockChallengeConditionList: [] });
export const GetGameLVConditionOpen   = async (data: any) => ({ length: 0, gameLinkedVerseConditionOpenList: [] });
export const GetGameLVConditionUnlock = async (data: any) => ({ length: 0, gameLinkedVerseConditionUnlockList: [] });
